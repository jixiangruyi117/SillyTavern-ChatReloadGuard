...
const SUPPORTED_VERSIONS = new Set(['1.18.0', '1.19.0']);
...
function createChatFingerprint(messages) {
    let hash = 2166136261;
    for (const message of Array.isArray(messages) ? messages : []) {
        hash = updateFingerprint(hash, message?.mes);
        hash = updateFingerprint(hash, message?.name);
        hash = updateFingerprint(hash, message?.send_date);
        hash = updateFingerprint(hash, message?.is_user);
        hash = updateFingerprint(hash, message?.is_system);
        hash = updateFingerprint(hash, message?.swipe_id);
        hash = updateFingerprint(hash, message?.extra?.gen_id);
    }
    return `${Array.isArray(messages) ? messages.length : 0}:${hash >>> 0}`;
}
...
export function createChatSnapshot(context) {
    ...
    return {
        ...
        fingerprint: createChatFingerprint(context.chat),
    };
}
...
function validateLoadedChat(data, snapshot) {
    ...
    if (receivedCount === snapshot.messages.length && createChatFingerprint(data.slice(1)) !== snapshot.fingerprint) {
        return { valid: false, reason: 'same-length-chat-mismatch', receivedCount };
    }
...
async function restoreSnapshot(getContext, snapshot, force = false) {
    ...
    const sameLengthMismatch = context.chat.length === snapshot.messages.length
        && createChatFingerprint(context.chat) !== snapshot.fingerprint;
    if (!force && context.chat.length > snapshot.messages.length && !sameLengthMismatch) {
        return false;
    }
    if (!force && context.chat.length === snapshot.messages.length && !sameLengthMismatch) {
        return false;
