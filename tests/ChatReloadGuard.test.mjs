...
test('enables on the audited vulnerable 1.18.0 and 1.19.0 flows', () => {
    ...
    assert.equal(future.status, 'compatible');
...
test('returns the snapshot when a same-length load contains different messages', async () => {
    const snapshot = createChatSnapshot(makeContext());
    const guarded = createGuardedFetch({
        ...
        downstreamFetch: async () => Response.json([
            ...
            { mes: 'message-0' },
            { mes: 'message-1' },
            { mes: 'corrupted-message-2' },
        ]),
    });

    const response = await guarded.guardedFetch(...chatRequest());
    assert.equal(response.headers.get('X-ST-Chat-Reload-Guard'), 'same-length-chat-mismatch');
    assert.deepEqual((await response.json()).slice(1), snapshot.messages);
});
...
test('does not inspect unrelated request bodies during a guarded reload', async () => {
    const snapshot = createChatSnapshot(makeContext());
    let bodyRead = false;
    const { guardedFetch } = createGuardedFetch({
        snapshot,
        downstreamFetch: async () => new Response('ok'),
    });

    await guardedFetch('/api/settings/get', {
        method: 'POST',
        get body() {
            bodyRead = true;
